...
impl UtilGenerator {
    ...
    pub fn i_store(
        ...
        let name = format!("I{target_bits}Store{source_bits}_{sign}");
        ...
        if !self.has(&name) {
            let body = abs(["offset"], {
                ...
                b.def(
                    "bytes",
                    self.num_to_be_bytes(var("val"), target_bits, source_bits),
                );
